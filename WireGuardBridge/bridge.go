...
func wgStartVKBootstrap(proxyConfigJSON *C.char) C.int32_t {
	...
	// Proxy is created here, but NOT started here.
	//
	// Starting p.Start() before setTunnelNetworkSettings + wgAttachWireGuard
	// races iOS VPN route/utun setup and produces stable DTLS handshake
	// timeouts with active-conns=0. The proxy will be started later by
	// TURNBind.Open() during wgAttachWireGuard/dev.Up().
	log.Printf("wgStartVKBootstrap: proxy created; start deferred until wgAttachWireGuard")
...
log.Printf("wgAttachWireGuard: dev.Up will open TURNBind and start proxy")
...
//export wgForceReconnect
func wgForceReconnect(tunnelHandle C.int32_t) {
id := int32(tunnelHandle)
...
log.Printf("wgForceReconnect: tunnel %d forcing reconnect", id)
entry.proxy.ForceReconnect()
}
