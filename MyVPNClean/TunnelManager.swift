...
struct TunnelStats: Codable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case lastHandshakeSec = "last_handshake_sec"
        case reconnects
...
class TunnelManager: ObservableObject {
    ...
    private var lastLoggedReconnects: Int64?
    private var lastLoggedInternetOK: Bool?
    ...
    func connect(config: TunnelConfig) async {
        SharedLogger.shared.log("[AppDebug] CONNECT BUTTON PRESSED — build telemetry check")
        ...
        do {
            ...
            let shared = UserDefaults(suiteName: "group.alex.MyVPNClean")
            ...
            proto.providerBundleIdentifier = "alex.MyVPNClean.MyVPNVKTurnTunnel"
    ...
    /// Send debug log message to both shared app log and extension log.
    private func debugLog(_ message: String) {
        VPNDebugLog.shared.log("[AppDebug] \(message)", category: .manager)
        SharedLogger.shared.log("[AppDebug] \(message)")
    ...
    ///     `group.alex.MyVPNClean` App Group entitlement, and
    ...
    private func checkStalledTunnelWatchdog(_ newStats: TunnelStats) {
        ...
        let noActiveConnections = newStats.activeConns == 0
        ...
        if stalledTunnelSince == nil {
            stalledTunnelSince = now
            debugLog("watchdog: stalled tunnel suspected — activeConns=0 rxRate=0 txRate=0 turnRTT=\(Int(newStats.turnRTTms)) uptime=\(newStats.tunnelUptimeSec)")
            return
        }
        ...
        debugLog("watchdog: stalled tunnel for \(Int(stalledFor))s — requesting force_reconnect")
        requestForceReconnect(reason: "stalled_tunnel_watchdog")
    }

    private func requestForceReconnect(reason: String) {
        guard let session = manager?.connection as? NETunnelProviderSession else {
            debugLog("watchdog: force_reconnect skipped — no NETunnelProviderSession")
            return
        }

        guard let msg = "force_reconnect".data(using: .utf8) else { return }

        do {
            try session.sendProviderMessage(msg) { [weak self] response in
                let text = response.flatMap { String(data: $0, encoding: .utf8) } ?? "nil"
                Task { @MainActor in
                    self?.debugLog("watchdog: force_reconnect response=\(text) reason=\(reason)")
                }
            }
        } catch {
            debugLog("watchdog: force_reconnect send failed — \(error.localizedDescription)")
        }
    }

    private func logStatsTelemetry(_ newStats: TunnelStats) {
        if lastLoggedActiveConns != newStats.activeConns {
            let old = lastLoggedActiveConns.map(String.init) ?? "nil"
            debugLog("telemetry: activeConns \(old) -> \(newStats.activeConns)/\(VKTurnSettings.numConnections)")
            lastLoggedActiveConns = newStats.activeConns
        }

        if lastLoggedReconnects != newStats.reconnects {
            let old = lastLoggedReconnects.map(String.init) ?? "nil"
            let delta = lastLoggedReconnects.map { newStats.reconnects - $0 } ?? 0
            debugLog("telemetry: reconnects \(old) -> \(newStats.reconnects) delta=\(delta)")
            lastLoggedReconnects = newStats.reconnects
        }
    }

    private func refreshInternetRTT() {
        let start = Date()
            ...
            try session.sendProviderMessage(msg) { [weak self] response in
                Task { @MainActor in
                    ...
                    guard let data = response else {
                        self.failedStatsPolls += 1
                        self.debugLog("watchdog: stats response nil (failedStatsPolls=\(self.failedStatsPolls))")
                    ...
                    guard let newStats = try? JSONDecoder().decode(TunnelStats.self, from: data) else {
                        self.failedStatsPolls += 1
...
self.objectWillChange.send()
                        self.logStatsTelemetry(newStats)
                        self.checkStalledTunnelWatchdog(newStats)
                        self.refreshInternetRTT()
